        ...
        <div style="margin-top: 10px">
          ...
          <el-table ref="refTable" :data="userList" border height="260px">
            ...
            <el-table-column label="用户名称" prop="user_name" :show-overflow-tooltip="true">
              <template #default="scope">
                <el-avatar size="small">{{ scope?.row?.user_name.substring(0, 1) }}</el-avatar>
                {{ scope?.row?.user_name }}
              </template>
            </el-table-column>
            <el-table-column label="用户编码" prop="user_code" :show-overflow-tooltip="true" />
...
watch(
  ...
  (newVal) => {
    if (userList.value.length > 0) {
      userList.value = userListOri.value
      if (newVal.length > 0) {
        for (const v of newVal) {
          userList.value = userList.value.filter((item) => item.user_id !== v.user_id)
        }
      }
    }
  },
...
// 查询表数据
function getList() {
  getDictDataApi({ dictCode: 'sys_user', params: { ...queryParams } }).then((res) => {
    userList.value = res.data.data
    userListOri.value = res.data.data
  })
}
...
function handleSelectUser(row) {
  ...
  if (checkedUsersList.value.some((c) => c.user_id === row.user_id)) {
...
function handleRemove(row) {
  if (checkedUsersList.value.some((c) => c.user_id === row.user_id)) {
    checkedUsersList.value = checkedUsersList.value.filter((item) => item.user_id !== row.user_id)
  } else {
